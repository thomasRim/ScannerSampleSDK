...
extension ViewController {
    ...
    func setupSLAM() {
        ...
        // Initialize the camera pose tracker.
        _slamState.tracker = try! STTracker.init(scene: _slamState.scene!, options: trackerOptions)
		...
		_slamState.cameraPoseInitializer = try! STCameraPoseInitializer.init(volumeSizeInMeters: _slamState.volumeSizeInMeters, options: [kSTCameraPoseInitializerStrategyKey: STCameraPoseInitializerStrategy.tableTopCube.rawValue])
		...
		_slamState.keyFrameManager = try! STKeyFrameManager.init(options: keyframeManagerOptions)
		
		_depthAsRgbaVisualizer = try! STDepthToRgba.init(options: [kSTDepthToRgbaStrategyKey: STDepthToRgbaStrategy.gray.rawValue])
